...
public class MLSparse extends MLNumericArray<Double> {
  ...
  /** APRO modification: Returns Nx2 array of non-null elements */
  public int[][] getIndices() {
    int inds[][] = new int[indexSet.size()][2];
    int i = 0;
   ...
   * Gets row indices
   *
   * <tt>ir</tt> points to an integer array of length nzmax containing the row
   * indices of the corresponding elements in <tt>pr</tt> and <tt>pi</tt>.
   */
  public int[] getIR() {
   ...
   * Gets column indices.
   *
   * <tt>jc</tt> points to an integer array of length N+1 that contains column
   * index information. For j, in the range <tt>0&lt;=j&lt;=N�1</tt>,
   * <tt>jc[j]</tt> is the index in ir and <tt>pr</tt> (and <tt>pi</tt>
   * if it exists) of the first nonzero entry in the jth column and
   * <tt>jc[j+1]�1</tt> index of the last nonzero entry. As a result,
   * <tt>jc[N]</tt> is also equal to nnz, the number of nonzero entries in the
   * matrix. If nnz is less than nzmax, then more nonzero entries can be
   * inserted in the array without allocating additional storage
   *
   * @return
   */
  public int[] getJC() {
  ...
  /**
   * Returns the imaginary part (PI) array. PI has length
